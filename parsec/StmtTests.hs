...
test2 = TestCase  $ (assertEqual "toto := 1; titi := 2;" ([("titi",2),("toto",1)]) (evalStmt manyAssign []))

ifthenelseTrue = parseString "(   toto := 0;   if (true) then       (toto :=1)     else       (toto := 2) )"


test3 = TestCase  $ (assertEqual "toto := 0; if true then toto :=1; else toto := 2;" ([("toto",1)]) (evalStmt ifthenelseTrue [])) 


ifthenelseEq = parseString "( toto := 0; if (toto == 0) then (toto :=1) else (toto := 2)) "

test4 = TestCase  $ (assertEqual "toto := 0; if toto == 0 then toto :=1; else toto := 2;" ([("toto",1)]) (evalStmt ifthenelseEq []))

while = parseString "toto := 0; while (toto < 10) do (toto := toto + 1)"

test5 = TestCase  $ (assertEqual "toto := 0; while (toto < 10) do (toto := toto + 1)" ([("toto",10)]) (evalStmt while []))
